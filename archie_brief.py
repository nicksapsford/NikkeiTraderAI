...
def _guinevere_kind(system_name):
    ...
    module is importable in-process (Gold/Oil/Gas AND Nikkei, which gained a working
    news module on 18-19 Jul 2026); otherwise calendar-only (US, Crypto), else none.

    Fix (21 Jul 2026): Nikkei was hardcoded to 'calendar' even though guinevere_news
    ...
    if any(x in n for x in ("gold", "oil", "gas", "nikkei")):
        return "news"
...
def build_system_brief(state, system_name, asset_label, logs_dir=None, now_utc=None):
    ...
    for pk in ("gas_price_usd", "oil_price_usd", "gold_price_usd",
               "nikkei_level", "us_level", "price", "current_price"):
