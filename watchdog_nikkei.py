"""
NikkeiTrader AI -- watchdog_nikkei.py  (Galahad)
Monitors main_nikkeitrader.py and automatically restarts on crash or freeze.
...
MAIN_SCRIPT   = BASE_DIR / "main_nikkeitrader.py"
LOG_FILE      = BASE_DIR / "logs" / "watchdog_nikkei.log"
...
log = logging.getLogger("NikkeiTrader.Galahad")
    ...
    try:
        requests.post(
            ...
            data={"token": _PO_TOKEN, "user": _PO_USER,
                  "title": f"[Nikkei] {title}", "message": message, "priority": priority},
            timeout=5,
        )
    except Exception:
        pass
...
class Watchdog:
    ...
    def _launch(self) -> None:
        ...
        log.info("NikkeiTrader AI started (PID %d)", self._proc.pid)
    ...
    def _check_restart_flag(self) -> bool:
        ...
        if RESTART_FLAG.exists():
            ...
            log.info("restart.flag detected -- restarting NikkeiTrader AI")
            return True
        return False
    ...
    def _do_restart(self, reason: str) -> bool:
        ...
        if self._alive():
            ...
            log.info("NikkeiTrader AI restarted (PID %d)", self._proc.pid)
    ...
    def _handle_limit_exceeded(self) -> None:
        self._prune_restarts()
        msg = (
            f"NikkeiTrader AI crashed {MAX_RESTARTS}x in 1 hour. "
    ...
    def _maybe_heartbeat(self) -> None:
        ...
        log.info(
            "Galahad heartbeat -- NikkeiTrader AI running for %.1f hours",
            uptime_h,
        )
        self._heartbeat_at = time.monotonic()
    ...
    def run(self) -> None:
        log.info("=" * 60)
        log.info("  NikkeiTrader AI -- Galahad Watchdog")
        ...
        try:
            ...
            log.error("FATAL: Could not start NikkeiTrader AI: %s", exc)
            return
    ...
    def shutdown(self) -> None:
        ...
        _push("Shutdown", "NikkeiTrader AI stopped cleanly.")
