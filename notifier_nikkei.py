...
log = logging.getLogger("NikkeiTrader.Percival")
...
def notify_trade_opened(
    ...
    _send(
        title   = "[Nikkei] Trade Opened -- NikkeiTrader AI",
        message = (
...
def notify_trade_closed_win(
    ...
    _send(
        title   = "[Nikkei] Trade WON -- NikkeiTrader AI",
        message = (
...
def notify_trade_closed_loss(
    ...
    _send(
        title   = "[Nikkei] Trade Lost -- NikkeiTrader AI",
        message = (
            f"{direction} closed at {exit_price:,.1f}\n"
...
def notify_kill_switch_triggered(
    ...
    _send(
        title   = f"[Nikkei] KILL SWITCH Tier {tier} -- NikkeiTrader AI",
        message = (
            f"{reason}\n"
...
def notify_kill_switch_reset(tier: int, wait_hours: int, capital: float) -> None:
    _send(
        title   = "[Nikkei] Trading Resuming -- NikkeiTrader AI",
        message = (
            f"Kill switch reset after {wait_hours}h cooldown (Tier {tier}).\n"
            f"Capital: £{capital:.2f}. Watching for Nikkei setups."
        ),
    )
...
def notify_tier3_urgent() -> None:
    _send(
        title   = "[Nikkei] URGENT: Manual Review Required",
        message = (
            "NikkeiTrader AI kill switch triggered 3x in 48 hours.\n"
...
def notify_system_startup(capital: float, mode: str = "PAPER") -> None:
    _send(
        title   = f"[Nikkei] NikkeiTrader AI Started ({mode})",
        message = (
            f"NikkeiTrader AI is live.\n"
            f"Capital: £{capital:.2f} | Mode: {mode}\n"
            f"Trading Japan 225 via IG Markets spread betting."
        ),
    )
...
def notify_system_shutdown(capital: float) -> None:
    _send(
        title   = "[Nikkei] NikkeiTrader AI Shutdown",
        message = (
            f"NikkeiTrader AI stopped cleanly.\n"
...
def notify_calendar_block(event_name: str, mins_remaining: int) -> None:
    _send(
        title   = "[Nikkei] Calendar Block Active",
        message = (
...
def notify_daily_summary(
    ...
    _send(
        title   = f"[Nikkei] Daily Summary {date_str}",
        message = (
...
def notify_milestone_review(milestone_num: int) -> None:
    _send(
        title   = f"[Nikkei] Milestone Review #{milestone_num} -- Arthur",
        message = (
...
def notify_system_error(error_msg: str) -> None:
    _send(
        title   = "[Nikkei] System Error -- NikkeiTrader AI",
